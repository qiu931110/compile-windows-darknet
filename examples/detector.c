...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
	...
	while (1) {
		if (filename) {
			...
			{ 
			fflush(stdout);
			input2 = fgets(input2,256,fp);  //读取一行
			if (!input2) return;
			strtok(input2, "\n");
			printf("%s\n", input2); //输出
...
void test_batch_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
	...
	while (1) {
		if (filename) {
			...
			int m = plist->size;
			if (access("G:\darknet-master\results", 0) == -1)//"/home/FENGsl/darknet/data"修改成自己的路径
			{
				if (mkdir("G:\darknet-master\results", 0777))//"/home/FENGsl/darknet/data"修改成自己的路径
				{
			...
			for (i = 0; i < m; ++i) {
				...
				if (outfile) {
					...
					char b[2048];
					sprintf(b, "G:\darknet-master\results\%s", GetFilename(path));//"/home/FENGsl/darknet/data"修改成自己的路径
